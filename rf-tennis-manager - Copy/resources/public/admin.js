...
function processAdminRequest(form_id, uri, title) {
    if ($(form_id).valid()) {
       rf_tennis_manager.tennismgr.db_update_request(form_id, uri, title, "admin");
        $("#" + admin_current_form_id).css("display", "none");
...
function team_schedule_all() {
   rf_tennis_manager.tennismgr.schedule-all($("#season-id option:selected").val(), $("#team-id option:selected").val());
};
