...
function change_to_email_form(match_id) {
    $("#send-availability-email").addClass("avail-small");
     rf_tennis_manager.matches.email_form(match_id);
...
function set_lineup(match_id) {
    $("#set-lineup").addClass("avail-small");
     rf_tennis_manager.matches.set_lineup(match_id);
    change_match_form("set-lineup");
}

function player_selected(selected_value) {
    return  rf_tennis_manager.matches.player_selected(selected_value);
}

function get_roster_data() {
    $("#sr_team_name").text($("#team_id option:selected").text());
     rf_tennis_manager.roster.roster($("#team_id option:selected").val());
}

function update_player_form(player_id) {
     rf_tennis_manager.roster.load_update_player_form(player_id);
    change_match_form("update-player");
}

function updateForfeitBtns(elem) {
     rf_tennis_manager.matches.forfeit_selected(elem.id, elem.value);
}
...
$().ready(function () {
    ...
    $("#c1-p2xx").mousedown(function (e) {
        if ( rf_tennis_manager.matches.player_selected(e.target.valueOf().value)) {
            e.preventDefault();
        }
    });
});
...
function processMatchRequest(form_id, uri, title) {
    if ($(form_id).valid()) {
       rf_tennis_manager.tennismgr.db_update_request(form_id, uri, title, "match");
        $("#" + match_current_form_id).css("display", "none");
